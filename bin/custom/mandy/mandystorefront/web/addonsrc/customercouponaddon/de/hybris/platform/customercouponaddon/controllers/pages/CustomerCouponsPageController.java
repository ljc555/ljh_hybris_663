...
public class CustomerCouponsPageController extends AbstractSearchPageController
{
	...
	public String claim(@PathVariable("code") final String code, final Model model, final RedirectAttributes redirectModel)
			throws CMSItemNotFoundException
	{
		final String returnMsg = customerCouponFacade.grantCouponAccessForCurrentUser(code);

		GlobalMessages.addFlashMessage(redirectModel, getGlobalMessageHolder(returnMsg), returnMsg, null);
	...
	/**
	 * Get return global message type by assign status
	 *
	 * @param returnMsg
	 *           assign status
	 * @return global message type
	 */
	protected String getGlobalMessageHolder(final String returnMsg)
	{
		switch (returnMsg)
		{
			case SUCCESS:
				return GlobalMessages.CONF_MESSAGES_HOLDER;
			case EXIST:
